...
fun TasksScreen(vm: TasksViewModel = hiltViewModel()) {
    ...
    if (showAdd) AddEditTaskDialog(null, { showAdd = false }) { t, d, p, _ -> vm.add(t, d, p, null); showAdd = false }
    editTask?.let { task ->
        AddEditTaskDialog(task, { editTask = null }) { t, d, p, _ ->
            vm.update(task.copy(title = t, description = d, priority = p)); editTask = null
        }
    }
}
...
private fun AddEditTaskDialog(task: TaskEntity?, onDismiss: () -> Unit, onConfirm: (String, String?, String, Long?) -> Unit) {
    ...
    AlertDialog(onDismissRequest = onDismiss,
        ...
        confirmButton = { FatumButton(if (task == null) "Crear" else "Guardar", enabled = title.isNotBlank(),
            onClick = { if (title.isNotBlank()) onConfirm(title.trim(), desc.ifBlank { null }, priority, null) }) },
